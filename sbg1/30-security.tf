...
resource "openstack_networking_secgroup_rule_v2" "httpv6" {
  direction         = "ingress"
  ethertype         = "IPv6"
  protocol          = "tcp"
  port_range_min    = 80
...
resource "openstack_networking_secgroup_rule_v2" "httpsv6" {
  direction         = "ingress"
  ethertype         = "IPv6"
  protocol          = "tcp"
  port_range_min    = 443
...
resource "openstack_networking_secgroup_rule_v2" "sshv6" {
  direction         = "ingress"
  ethertype         = "IPv6"
  protocol          = "tcp"
  port_range_min    = 22
